...
class ResumeShortlisterAgent(BaseAgent):
    ...
    async def _arun(
        ...
        logger.info("shortlister_start", num_resumes=len(resume_file_ids), top_n=n)
        ...
        for i, res in enumerate(parsed_results):
            if isinstance(res, Exception):
                logger.warning("resume_parse_failed", file_id=resume_file_ids[i], error=str(res))
                continue
        ...
        # ── Step 2: 候选人评估（原版 match_score 0-10） ──────────────────
        ...
        logger.info("scoring_complete", candidates_count=len(selected_indexes))
