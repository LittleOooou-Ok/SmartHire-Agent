...
async def decide_shortlist(session_id: str, body: HITLDecisionRequest):
    ...
    try:
        ...
        logger.error("hitl_shortlist_error", session_id=session_id, error=str(exc))
        raise HTTPException(status_code=500, detail=f"处理审批时出错: {exc}")

    return {"session_id": session_id, "decision": body.decision, "message": "决策已记录，工作流恢复中。"}
...
async def decide_pre_screening(session_id: str, body: HITLDecisionRequest):
    ...
    try:
        ...
        logger.error("hitl_pre_screening_error", session_id=session_id, error=str(exc))
        raise HTTPException(status_code=500, detail=f"处理审批时出错: {exc}")
...
async def decide_onboarding(session_id: str, body: OnboardingDecisionRequest):
    ...
    try:
        ...
        logger.error("hitl_onboarding_error", session_id=session_id, error=str(exc))
        raise HTTPException(status_code=500, detail=f"处理入职选择时出错: {exc}")

    return {
        "session_id": session_id,
        "selected_count": len(body.selected_candidate_ids),
        "message": "入职决策已记录，邮件将发送给选定的候选人。",
    }
