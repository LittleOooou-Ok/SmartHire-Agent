...
async def approve_shortlist(session_id: str, feedback: str | None = None) -> None:
    ...
    # 如果数据库中有预筛选结果（来自网页对话），注入 graph checkpoint
    try:
        session = await db.get_session(session_id)
        if session:
            snap = session.get("state_snapshot", {})
            existing_results = snap.get("pre_screening_results", [])
            if existing_results:
                state_update["pre_screening_results"] = existing_results
                state_update["current_step"] = "pre_screening_complete"
                logger.info("injecting_pre_screening_results", count=len(existing_results))
    except Exception as exc:
        logger.warning("inject_results_failed", error=str(exc))
...
async def _run_graph(graph, config: dict, session_id: str) -> None:
    ...
    try:
        ...
        async for event in graph.astream(None, config, stream_mode="values"):
            ...
            logger.info("graph_event", session_id=session_id, step=step, event_num=event_count)
            try:
                update_data = {k: v for k, v in event.items() if k != "messages"}
                await db.update_session(session_id, update_data)
            except Exception as db_exc:
                logger.error("graph_db_update_failed", session_id=session_id, error=str(db_exc))

        if event_count == 0:
            logger.warning("graph_no_events", session_id=session_id, detail="graph returned no events, may be stuck")

        logger.info("graph_completed_or_paused", session_id=session_id, total_events=event_count)
    except Exception as exc:
        logger.error("graph_resume_error", session_id=session_id, error=str(exc), error_type=type(exc).__name__)
        try:
